...
impl AppState {
    /// Loaded conversation, `entry_point`, or default-line / missing-file fallbacks. Returns
    /// `(conversation_id, node_id, line_index)` to enter dialogue, or `None` if nothing to show.
    fn resolve_npc_dialogue(
        conversation_id: &str,
        dialogue_cache: &mut dialogue::ConversationCache,
        world_state: &mut WorldState,
    ) -> Option<(String, String, u32)> {
        if let Some(conv) = dialogue_cache.get_or_load(conversation_id) {
            match dialogue::entry_point(conv, world_state) {
                Some((node_id, line_index)) => {
                    Some((conversation_id.to_string(), node_id, line_index))
                }
                None => conv.default_line.clone().and_then(|default_line| {
                    let fallback_id = format!("{}_fallback", conversation_id);
                    let temp_conv = dialogue::Conversation::one_line(&default_line);
            ...
            let temp_conv = dialogue::Conversation::one_line(MISSING_DIALOGUE_PLACEHOLDER);
            let (node_id, line_index) = dialogue::entry_point(&temp_conv, world_state)
        ...
        if *backpack_open {
            ...
            if !*last_near_npc {
                if let Some(interaction) = trigger {
                    ...
                    if let Some((conversation_id, node_id, line_index)) =
                        Self::resolve_npc_dialogue(&conversation_id, dialogue_cache, world_state)
                    {
    ...
    fn update_dialogue(
        ...
        let AppState::Dialogue {
            ...
            unreachable!("update_dialogue only called in Dialogue");
        };
