...
impl UiTheme {
    fn merge_backpack(&mut self, f: BackpackThemeFile) {
        if let Some(v) = f.panel_width { self.backpack_panel_width = v; }
        if let Some(v) = f.panel_height { self.backpack_panel_height = v; }
        if let Some(v) = f.padding { self.backpack_padding = v; }
        if let Some(v) = f.slot_height { self.backpack_slot_height = v; }
        if let Some(v) = f.border_top_px { self.backpack_border_top_px = v; }
        if let Some(v) = f.panel_fill { self.backpack_panel_fill = v; }
        if let Some(v) = f.panel_border { self.backpack_panel_border = v; }
        if let Some(v) = f.title { self.backpack_title = v; }
        if let Some(v) = f.slot_empty { self.backpack_slot_empty = v; }
        if let Some(v) = f.section_usable { self.backpack_section_usable = v; }
        if let Some(v) = f.section_weapon { self.backpack_section_weapon = v; }
        if let Some(v) = f.section_passive { self.backpack_section_passive = v; }
        if let Some(v) = f.row_weapon { self.backpack_row_weapon = v; }
        if let Some(v) = f.row_passive { self.backpack_row_passive = v; }
        if let Some(v) = f.row_equipped { self.backpack_row_equipped = v; }
    }
}
...
pub fn load_theme() -> Result<UiTheme, ThemeLoadError> {
    ...
    let s = std::fs::read_to_string(&path)
        .map_err(|e| ThemeLoadError::Io(e, path.clone()))?;
    let file: UiThemeFile = serde_json::from_str(&s)
        .map_err(|e| ThemeLoadError::Json(e, path.clone()))?;
