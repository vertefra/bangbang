...
**The Right Way:** The component should only store the String ID (e.g. `conversation_id: "mom_intro"`). Systems dynamically query the `AssetStore`, `WorldState`, or a cache to render the content when required.
...
**Carve-out:** A **narrow read-only** `World` query for trivial always-on HUD data (e.g. player `current`/`max` HP with no ECS mutation) is acceptable. **Heavy** UI (backpack lines, dialogue layout, formatting loops) still belongs in `App::update()`.
