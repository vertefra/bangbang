...
3. **UI panel quads** (`draw_ui_pass`): **Player HP HUD first** (top-left track, optional border, fill by ratio, numeric `current / max` label on `white_over` + `font`), then dialogue panel fill/border, then backpack panels (solid quads on the `white_over` batch). Backpack panel draws: golden "BACKPACK" title, three sections (Usable/Weapons/Passives) each with section header + slots (icon + label). **Overworld toast:** after the backpack block, if a transient message is active (e.g. blocked door `deny_message` from [maps.md](maps.md)), a bottom band reuses dialogue panel colors; layout: `layout::overworld_toast_band_rect` / `overworld_toast_text_pos`. Timer and string live on `App` in `main.rs` (`overworld_toast`), not in the renderer.
4. **Skill icons** (`skill_icons` batch): Skill icon PNGs from `assets/skills/{id}.skill_image.png` drawn as textured quads (slot-height square) to the left of each filled backpack slot. Drawn after `white_over` (panel bg) so icons sit on the panel, before `font` text.
...
6. **UI text** (`draw_ui_pass` + `font` batch): Dialogue lines and backpack labels via **vector text** (`fontdue` + Noto Sans, `gpu/text_atlas.rs`).
...
- Backpack helpers: `backpack_panel_title_y` (golden panel header), `backpack_usable_title_y` (shifted below the panel title), `backpack_hotkey_hint_y` (bottom hint line).
